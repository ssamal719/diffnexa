...
/**
 * Reads a policy page now and compares it with a baseline the person kept.
 *
 * The response is the same deterministic comparison Website Change Detector
 * produces, with clause topics alongside it. The baseline is passed straight to
 * the engine, which validates it strictly: an untrusted file should be parsed
 * in exactly one place, by the code that knows what a valid capture looks like.
 */
export async function POST(request: Request) {
  const decision = checkRateLimit("policy-compare", clientKey(request));
  if (!decision.allowed) {
  ...
  if (parsed.body.example === true) {
    return callEngine("/v1/policy/compare", { example: true }, "policy");
  }
  ...
  return callEngine("/v1/policy/compare", { url: url.trim(), previous_snapshot: previous }, "policy");
}
