/**
 * The website's Competitor Monitor routes.
 *
...
const SECRET = "competitor-secret-never-in-a-response";
...
async function post(route: "snapshot" | "compare", body: unknown, ip = "203.0.113.9") {
  vi.resetModules();
  const handler = await import(`@/app/api/competitor/${route}/route`);
  return handler.POST(
    new Request(`http://site.test/api/competitor/${route}`, {
      method: "POST",
...
const COMPARISON = {
  ...
  changes: [
    {
      ...
      competitorSignal: {
        ...
        basis: "money_value",
        matchedText: "$12",
        reason: "“$12” is an amount of money",
  ...
  competitor: {
    ...
    signals: [{ id: "pricing_commercial", label: "Pricing & Commercial", changeCount: 1, changeIds: ["c0"] }],
    changedSignals: ["pricing_commercial"],
  },
...
describe("capturing a competitor's page", () => {
  ...
  it("never forwards the competitor's name or the page type", async () => {
    stubEngine(200, { snapshot: SNAPSHOT });
    await post("snapshot", {
...
describe("checking a competitor's page", () => {
  it("calls the competitor endpoint and returns the signals with the changes", async () => {
    ...
    expect(response.status).toBe(200);
    expect(calls[0].url).toBe(`${ENGINE}/v1/competitor/compare`);
    expect(body.competitor.changedSignals).toEqual(["pricing_commercial"]);
    expect(body.changes[0].competitorSignal.summary).toBe("Touches Pricing & Commercial");
...
describe("rate limits", () => {
  it("has buckets of its own, with the same limits as the other page tools", () => {
    expect(LIMITS["competitor-capture"]).toEqual(LIMITS["web-capture"]);
    expect(LIMITS["competitor-compare"]).toEqual(LIMITS["web-compare"]);
  });

  it("refuses a burst of checks from one visitor", async () => {
    stubEngine(200, COMPARISON);
    // One module instance throughout, so the limiter remembers each request.
    const { POST } = await import("@/app/api/competitor/compare/route");
    const send = () =>
      POST(
        new Request("http://site.test/api/competitor/compare", {
          method: "POST",
    ...
    const perMinute = LIMITS["competitor-compare"][0].limit;
