...
type Held = {
  ...
  captured: PriceBaselineFile | null;
};

/**
 * A pricing or product page, checked against a baseline you kept.
 *
 * A baseline on the left, the page now on the right, and one button to
 * compare them. The product name and the page type are your labels: they go into
...
export function PriceDesk() {
  ...
  async function tryExample() {
    ...
    setState({
      ...
      url: EXAMPLE_PAGES.price.url,
      product: EXAMPLE_PAGES.price.product,
      pageType: EXAMPLE_PAGES.price.pageType,
  ...
  const baseline: DeskBaseline | null = held
    ? {
        ...
        details: [
          ...(held.label ? [{ label: "Product / service", value: held.label }] : []),
  ...
  return (
    <>
      <MonitorDesk
        fileInputId="price-baseline-file"
        pageFields={
          <>
            <TextField
        ...
        example={{ note: "Compares two saved versions of a fictional product’s pricing page — no address, no account and no baseline of your own needed.", onTry: tryExample }}
        footnote="DiffNexa reads the page once and discards it — and your baseline — when your result is ready. Nothing is stored, there is no account, and the comparison sends nothing to any AI service. DiffNexa reports what the page says, not whether a price is good or bad."
      />
      ...
      {state.name === "checked" && state.example && <ExampleGuide guide={EXAMPLE_GUIDES.price} />}
      {state.name === "checked" && (
        <ReportWithAnalyst key={state.run} tool="price" result={state.result} seal={state.seal}>
