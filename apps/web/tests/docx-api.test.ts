/**
 * The website's DOCX Compare route.
 *
...
const SECRET = "docx-secret-never-in-a-response";
const ENGINE = "http://engine.internal:8000";
const COMPARISON = JSON.parse(readFileSync(join(import.meta.dirname, "fixtures", "docx-comparison.json"), "utf8"));
...
function upload(files: Partial<Record<"original" | "revised", Uint8Array | string>>, ip = "203.0.113.7") {
  ...
  for (const [side, bytes] of Object.entries(files)) {
    ...
    else form.append(side, new File([bytes as BlobPart], `My Private ${side} Contract.docx`));
  }
  return new Request("http://site.test/api/docx/compare", {
    method: "POST",
    body: form,
...
afterEach(() => {
  ...
  delete process.env.DIFFNEXA_DOCX_MAX_FILE_MB;
});

describe("comparing two Word documents", () => {
  it("sends both files to the engine's DOCX endpoint and returns its result", async () => {
    ...
    expect(calls[0].url).toBe(`${ENGINE}/v1/docx/compare`);
  ...
  it("never forwards the person's own filenames", async () => {
    ...
    const sent = calls[0].init.body as FormData;
    expect((sent.get("original") as File).name).toBe("original.docx");
    expect((sent.get("revised") as File).name).toBe("revised.docx");
  });
...
describe("refusals", () => {
  ...
  it("refuses an empty file, naming which one", async () => {
    ...
    expect((await response.json()).error).toEqual({
      code: "docx_empty_file",
      message: DOCX_ERROR_MESSAGES.docx_empty_file,
      side: "revised",
    });
    expect(calls).toHaveLength(0);
  });
  ...
  it("refuses an oversized file before calling the engine", async () => {
    process.env.DIFFNEXA_DOCX_MAX_FILE_MB = "1";
    stubEngine(200, COMPARISON);
    const big = new Uint8Array(1024 * 1024 + 1);
    big.set(ZIP);
    const response = await post({ original: big, revised: ZIP });
    expect(response.status).toBe(413);
    expect((await response.json()).error).toMatchObject({ code: "docx_too_large", side: "original" });
    expect(calls).toHaveLength(0);
  });

  it("passes the engine's refusal on with the shared wording and the side", async () => {
    stubEngine(400, {
      error: { code: "docx_macro_enabled", message: "internal detail vbaProject.bin", side: "original" },
    });
    const response = await post({ original: ZIP, revised: ZIP });
    expect(response.status).toBe(400);
    const { error } = await response.json();
    expect(error).toEqual({
      code: "docx_macro_enabled",
      message: DOCX_ERROR_MESSAGES.docx_macro_enabled,
      side: "original",
    });
  });
...
describe("rate limits", () => {
  it("has a bucket of its own, with the PDF comparison limits", () => {
    expect(LIMITS["docx-compare"]).toEqual(LIMITS["pdf-compare"]);
  });

  it("refuses a burst of comparisons from one visitor", async () => {
    stubEngine(200, COMPARISON);
    const { POST } = await import("@/app/api/docx/compare/route");
    const perMinute = LIMITS["docx-compare"][0].limit;
