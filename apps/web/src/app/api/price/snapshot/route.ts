...
/**
 * Captures a pricing or product page as it reads today, for the person to keep.
 *
 * Capturing a pricing page is the same operation as capturing any page, so
 ...
 *
 * Only the address is forwarded. The product or service name and the page type
 * are the person's own labels, kept in their baseline file in the browser; they are
 * never sent to the engine and cannot change how the page is read.
 */
export async function POST(request: Request) {
  const decision = checkRateLimit("price-capture", clientKey(request));
  if (!decision.allowed) {
